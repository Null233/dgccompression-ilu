        ...
        for p, (handle, ctx) in self._handles.items():
            ...
            if isinstance(self._compression,topkCompressor) \
                ...
                for handle, sizes in zip(handles, tensor_sizes):
                    #print(f'22. handle:{handle}, sizes:{sizes}')
                    
                    gathered = synchronize_(handle)
                    #print(f"receive handle:{handle}, with sizes:{sizes}")
                    #print(gathered.size())
                    #print(f'33. gathered:{gathered}')
                    tensors_ag.append(gathered.split(sizes))
                list_tensor_decompressed = []
                for tensor_compressed in zip(*tensors_ag):
                    #print(f'44. tensor_compressed:{tensor_compressed}')
                    tensor_decompressed = self._compression.decompress(tensor_compressed, ctx)
